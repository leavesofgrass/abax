...
worker, :mod:`abax.console_worker`).

Pure stdlib + ``abax.core`` (no Qt), so it imports cleanly in a headless child.
...
class _LazyModule:
    ...
    def __repr__(self) -> str:
        ...
        return f"<module {name!r} (abax lazy — imports on first use)>"
...
def build_namespace(workbook, refresh=None) -> dict:
    ...
    return {
        ...
        "cluster": cluster, "nonparam": nonparam,
        "ml": ml, "trees": trees, "bayes": bayes,
