...
:data:`abax.core.functions.FUNCTIONS` / ``LAZY_FUNCTIONS`` registries, so any
user-defined functions installed by :mod:`abax.macros` show up automatically.
...
def _in_string(text: str, cursor: int) -> bool:
    # abax string literals are double-quoted; an odd number of quotes before
...
SIGNATURES = {
    ...
    # Circuit fundamentals — impedance args are "R+jX" strings or R, X numbers
    "OHMV": "OHMV(current_a, resistance_ohm)", "OHMI": "OHMI(voltage_v, resistance_ohm)",
    "OHMR": "OHMR(voltage_v, current_a)",
    "POWERVI": "POWERVI(voltage_v, current_a)", "POWERIR": "POWERIR(current_a, resistance_ohm)",
    "POWERVR": "POWERVR(voltage_v, resistance_ohm)",
    "RSERIES": "RSERIES(r1, [r2], …)", "RPARALLEL": "RPARALLEL(r1, [r2], …)",
    "LSERIES": "LSERIES(l1, [l2], …)", "LPARALLEL": "LPARALLEL(l1, [l2], …)",
    "CSERIES": "CSERIES(c1, [c2], …)", "CPARALLEL": "CPARALLEL(c1, [c2], …)",
    "TAURC": "TAURC(resistance_ohm, capacitance_f)",
    "TAURL": "TAURL(resistance_ohm, inductance_h)",
    "TCCHARGE": "TCCHARGE(time_s, tau_s)", "TCDECAY": "TCDECAY(time_s, tau_s)",
    "TCTIME": "TCTIME(fraction, tau_s)",
    "ZSERIESRLC": "ZSERIESRLC(freq_hz, r_ohm, l_h, c_f)",
    "ZPARALLELRLC": "ZPARALLELRLC(freq_hz, r_ohm, l_h, c_f)",
    "ZMAG": "ZMAG(z | r, [x])", "PHASEANGLE": "PHASEANGLE(z | r, [x])",
    "POWERFACTOR": "POWERFACTOR(z | r, [x])", "ADMITTANCE": "ADMITTANCE(z | r, [x])",
    "CONDUCTANCE": "CONDUCTANCE(z | r, [x])", "SUSCEPTANCE": "SUSCEPTANCE(z | r, [x])",
    "POLAR2RECT": "POLAR2RECT(magnitude, angle_deg)",
    "REALPOWER": "REALPOWER(v_rms, i_rms, phase_deg)",
    "REACTIVEPOWER": "REACTIVEPOWER(v_rms, i_rms, phase_deg)",
    "APPARENTPOWER": "APPARENTPOWER(v_rms, i_rms)",
    "QSERIES": "QSERIES(r_ohm, x_ohm)", "QPARALLEL": "QPARALLEL(r_ohm, x_ohm)",
    # Radio-system math
    "ERPW": "ERPW(power_w, gain_dbd, [loss_db], …)",
    "EIRPW": "EIRPW(power_w, gain_dbi, [loss_db], …)",
    "RXLEVEL": "RXLEVEL(ptx_dbm, gtx_dbi, grx_dbi, path_loss_db, [cable_loss_db=0])",
    "LINKMARGIN": "LINKMARGIN(rx_dbm, mds_dbm, [snr_db=0])",
    "BWNOISEDB": "BWNOISEDB(bw_from_hz, bw_to_hz)",
    "OPAMPINV": "OPAMPINV(rf_ohm, rin_ohm)", "OPAMPNONINV": "OPAMPNONINV(rf_ohm, rin_ohm)",
    "MODINDEX": "MODINDEX(deviation_hz, modulating_hz)",
    "DEVRATIO": "DEVRATIO(max_deviation_hz, max_modulating_hz)",
    "CARSONBW": "CARSONBW(deviation_hz, modulating_hz)",
    "CWBW": "CWBW(wpm, [k=5])", "FSKBW": "FSKBW(shift_hz, baud, [k=1.2])",
    "ADCBITS": "ADCBITS(range_v, resolution_v)", "ADCLEVELS": "ADCLEVELS(bits)",
    "ADCLSB": "ADCLSB(reference_v, bits)", "ADCSNR": "ADCSNR(bits)",
    "NYQUIST": "NYQUIST(max_freq_hz)",
    "USBMAXFREQ": "USBMAXFREQ(upper_edge_hz, [bandwidth_hz=3000])",
    "LSBMINFREQ": "LSBMINFREQ(lower_edge_hz, [bandwidth_hz=3000])",
    "LINELEN": "LINELEN(freq_hz, fraction_wl, [velocity_factor=1])",
    "ELECDEG": "ELECDEG(length_m, freq_hz, [velocity_factor=1])",
    "STUBX": "STUBX(z0_ohm, elec_len_deg, [open_end=FALSE])",
    "ANTEFF": "ANTEFF(r_radiation_ohm, r_loss_ohm)",
