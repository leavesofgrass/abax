...
def to_adif(records: list[dict], *, header: str = "abax ADIF export") -> str:
    """Emit ``records`` as an ADIF document with a header.

    The header is written as a comment line followed by ``<ADIF_VER:5>3.1.4``,
    ``<PROGRAMID:5>abax`` and ``<EOH>``. Each record's fields are emitted as
...
def load_adif(path):
    """Read an ADIF file into a :class:`~abax.core.sheet.Sheet` named "Log"
