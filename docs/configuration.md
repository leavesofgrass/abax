...
| `windowed_store_capacity` | int | `0` | Keep at most this many cells **resident per sheet** and spill the rest to a private temp file (`0` = off, the default — every cell stays in RAM). A **memory ↔ latency trade-off** applied when a file is opened. Worth it **only for very large data imports** (lots of literal cells): a ~50k-cell dataset roughly halves its peak memory. Formula-heavy sheets see little benefit (the value cache and key index stay resident). Dependency chains evaluate correctly at **any** capacity (up to ~10,000 cells deep — the engine's recursion headroom, independent of this setting), so chain depth does not constrain your choice. Note the switch itself briefly costs extra memory (~1.5× the plain sheet while cells migrate into the windowed store); the savings are the steady state after it. Set it from **Preferences → System → Performance** (or by hand in `settings.json`). |
