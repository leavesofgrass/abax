"""PyInstaller entry point for abax (the macOS .app main executable).

Same behavior as the installed `abax` script: no arguments opens the GUI, and
the whole CLI surface (view / convert / get / tui / doctor / ...) works when the
executable is run from a terminal. The --run-console-worker escape hatch lives
inside abax.app.main.
"""
